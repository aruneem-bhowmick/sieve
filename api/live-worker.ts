...
import { type LiveJob } from "./_lib/jobs";
import { secureEqual } from "./_lib/session";
...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (request.method !== "POST" || !workerSecret || !secureEqual(request.headers.get("x-sieve-worker") ?? "", workerSecret)) return new Response("Unauthorized", { status: 401 });
  const body = await request.json() as { job: LiveJob; submission: { tasks: SubmittedTask[] } };
  ...
  try {
    ...
    await sandbox.writeFiles(body.submission.tasks.flatMap((task) => [
      ...Object.entries(task.files).map(([path, content]) => ({ path: `/vercel/sandbox/tasks/${task.task_id}/${path}`, content })),
      { path: `/vercel/sandbox/tasks/${task.task_id}/package.json`, content: TRUSTED_PACKAGE },
    ]));
