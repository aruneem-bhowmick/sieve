...
Wave 3 additionally requires the root Node preflight `npm ci`, then a recorded SIEVE-T3 baseline run that applies its edit before executing the fixture command. The pristine fixture is intentionally failing and is not the acceptance command. On Node 22 Windows, a registry certificate trusted by Windows but not Node's bundled store may be used safely by setting `$env:NODE_USE_SYSTEM_CA = '1'` in the current PowerShell process before `npm ci`; it preserves TLS validation and does not persist configuration. Any remaining certificate-validation failure is an environment blocker: repair the trusted certificate chain before retrying, without changing repository dependencies or disabling TLS verification.
